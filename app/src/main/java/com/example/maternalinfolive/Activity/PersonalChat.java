...
import static com.example.maternalinfolive.Utils.HttpUrls.REPLY_MESSAGE;
...
import static com.example.maternalinfolive.Utils.HttpUrls.VIEW_MESSAGES_ADMIN;
...
public class PersonalChat extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List = new ArrayList<>();
        adapter = new Chat_Adapter(List, PersonalChat.this);
...
//        List.clear();
        StringRequest request = new StringRequest(Request.Method.POST, VIEW_MESSAGES_ADMIN, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {

                if (response.equalsIgnoreCase("empty")) {
                    Toast.makeText(PersonalChat.this, "Empty Message List", Toast.LENGTH_SHORT).show();
                }
                else {
            ...
            Toast.makeText(PersonalChat .this, error.getMessage(), Toast.LENGTH_LONG).show();
        }
        ){
            @Override
            protected Map<String, String> getParams() {
                ...
                params.put("client",getIntent().getExtras().getString("destination"));
                return params;

            }
        };

        RequestQueue requestQueue = Volley.newRequestQueue(PersonalChat.this);
    ...
    private void getMessageImage() {
        try {
            ...
            CropImage.activity().setGuidelines(CropImageView.Guidelines.ON).setAspectRatio(1, 1).start(PersonalChat.this);
        ...
        class AsyncTaskUploadClass extends AsyncTask<String,Void,String> {
            ...
            protected void onPostExecute(String string1) {
                ...
                if(string1.equals("Saved") || string1.contains("Saved")){
                    Toast.makeText(PersonalChat.this, "Message Successfully", Toast.LENGTH_LONG).show();
                    msg.setText("");
                    imageUri=null;
                    mesg_img.setImageURI(null);
                    image_lay.setVisibility(View.GONE);
                    ...
                    Toast.makeText(PersonalChat.this, ""+string1, Toast.LENGTH_SHORT).show();
                }
            }
            @Override
            protected String doInBackground(String... params) {
                ...
                hashMap.put("destination", params[3]);
                finalResult = httpParse.ImageHttpRequest(REPLY_MESSAGE,hashMap);
                return finalResult;
            }
        }
        AsyncTaskUploadClass userRegisterFunctionClass = new AsyncTaskUploadClass();
        userRegisterFunctionClass.execute(user,msgs,file,getIntent().getExtras().getString("destination"));
    }
